...
namespace WebAPIOEE.Infrastructure.Core
{
    ...
    {
        private readonly OEEContext _entities;
        ...
        protected GenericRepositoryEntities(OEEContext entities)
        {
            _entities = entities;
        ...
        /// <param name="predicate">Expretion type Lambda</param>
        /// <returns>Return a List type T</returns>
        public async Task<T> SingleFindBy(Expression<Func<T, bool>> predicate)
        {
            return await Dbset.FirstOrDefaultAsync(predicate);
            //return await Dbset.FirstOrDefaultAsync(predicate).ToListAsync();
            //var task = Task.Run(() => Dbset.Where(predicate).ToList());
            //return await task;
